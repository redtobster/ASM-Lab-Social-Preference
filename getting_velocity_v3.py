...
def compute_velocity_df(df, tank_length, tank_width, direction):
	...
	# counter = 0
	# acc_value = 0
	# for i in range(index_x[0]+1, len(df)):
	#     if counter == frame_rate:
	#         counter = 0
	#         if acc_value < (body_length * 0.1):
	#         	if direction == 'left':
	...
	# 	        while (acc_value < body_length * 0.1):
	# 	            next(i)
	# 	            acc_value = acc_value + df.loc[i+counter, index_fps[1]]
	# 	        freeze_duration = (frame_rate + counter - 1)/frame_rate
	# 	        freeze_info.append((quad, freeze_duration, i))
	# 	        counter = 0
	#         lst.append(acc_value)
	#         acc_value = 0
	#     else:
