...
from classifying_positions_v3 import assign_left_tank, assign_right_tank
import os
import pandas as pd
import glob
import numpy as np
...
directory = path + 'extracted_info_' + output_folder + '_2mins'
if not os.path.exists(directory):
    os.makedirs(directory)
...
# creating the csv file for the first excel file to have the header
# opening the excel file
file_name = path + first_two_minutes_left[0]
df = pd.read_excel(io=file_name, sheet_name='Tank1', header=None)
...
# assigning the x, y positions of the fish to different quadrants (left tank)
for i in range(1, len(first_two_minutes_left)):
    # opening the excel file
    file_name = path + first_two_minutes_left[i]
    df = pd.read_excel(io=file_name, sheet_name='Tank1', header=None)
    # using the function to extract the values
...
# assigning the x, y positions of the fish to different quadrants (right tank)
for i in range(0, len(first_two_minutes_right)):
    # opening the excel file
    file_name = path + first_two_minutes_right[i]
    df = pd.read_excel(io=file_name, sheet_name='Tank1', header=None)
